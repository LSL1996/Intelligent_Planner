...
namespace Intelligent_planner{
    ...
    private:
        ...
        /* state matrix*/
        Eigen::Matrix<double, 6, 6> At_, Phi_;
        Eigen::Matrix<double, 6, 1> Bt_;
        Eigen::Matrix<double, 6, 3> Dt_;
        Eigen::Matrix<double, 1, 6> Kt_;
        /* dim */
        const static int nx = 6;
        const static int nu = 1;
        const static int nw = 3;
        Eigen::Vector3d w_noise_;
        ...
        void updateMatrix(Eigen::Vector3d& acc, Eigen::Vector3d& vel, Eigen::Vector3d& noise);
