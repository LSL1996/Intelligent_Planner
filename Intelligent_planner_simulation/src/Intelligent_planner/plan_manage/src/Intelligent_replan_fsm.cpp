...
void IntelligentReplanFsm::init(ros::NodeHandle& nh){
    ...
    ego_traj_pub_            = nh.advertise<visualization_msgs::Marker> ("/ego_traj", 50);
    ego_opt_pub_            = nh.advertise<visualization_msgs::Marker> ("/ego_opt", 50);
...
bool IntelligentReplanFsm::kinoNMPCtraj()
{
    ...
    if(mpc_traj_acc_.size() != 0){
        double t_cur = (ros::Time::now() -tMpc1_).toSec();
        int k = floor(t_cur/mpc_delta_T_);
        Eigen::Vector3d acc_l = mpc_traj_acc_[k];
        Eigen::Vector3d acc_r = mpc_traj_acc_[k+1];
    ...
    planner_manager_->external_acc_ = external_force_remain_;
    ...
    if(search_success){
        ...
        if (have_traj_){
            tMpc2_ = ros::Time::now();
            // double t = (tMpc2_ - tMpc1_).toSec();
            double t = (tMpc2_ - tMpc1_).toSec() + mpc_opt_;
            int k = floor(t/mpc_delta_T_);
            t = t - k * mpc_delta_T_;

            Eigen::Vector3d pos_L, pos_R, vel_L, vel_R, acc_L, acc_R, jerk_L, jerk_R;
            acc_L = planner_manager_->mpc_traj_acc_[k];
            acc_R = planner_manager_->mpc_traj_acc_[k+1];
            vel_L = planner_manager_->mpc_traj_vel_[k];
            vel_R = planner_manager_->mpc_traj_vel_[k+1];
            pos_L = planner_manager_->mpc_traj_pos_[k];
            pos_R = planner_manager_->mpc_traj_pos_[k+1];
            ...
            // record the last mpc
            tMpc1_lst_ = tMpc1_;
            tMpc1_ = tMpc2_ + ros::Duration(mpc_opt_);

            mpc_traj_pos_ = planner_manager_->mpc_traj_pos_;
            mpc_traj_vel_ = planner_manager_->mpc_traj_vel_;
            mpc_traj_acc_ = planner_manager_->mpc_traj_acc_;
            bool mpc_plan_success = planner_manager_->highMpc(start_state, if_external_force_adaptive_, external_force_);
            if (mpc_plan_success){
                if(have_times == 0){
                    have_times =1;
                }
...
void IntelligentReplanFsm::calculateExtForceAssignWeight(){
    ...
    if(mpc_traj_acc_.size()!=0){
        acc_l = mpc_traj_acc_[k];
        acc_r = mpc_traj_acc_[k+1];
        t_cur = t_cur - k * mpc_delta_T_;
        start_acc(0) = acc_l(0) + (acc_r(0) - acc_l(0)) * t_cur / mpc_delta_T_;
        start_acc(1) = acc_l(1) + (acc_r(1) - acc_l(1)) * t_cur / mpc_delta_T_;
        start_acc(2) = acc_l(2) + (acc_r(2) - acc_l(2)) * t_cur / mpc_delta_T_;
    ...
    if((abs(external_force_(0)) || abs(external_force_(1)) || abs(external_force_(2)))> noise_boundary_){
        ...
        for(int j = 0; j < 5; j++) {
            vel_temp = vel_t[j];
            int occupied = planner_manager_->sdf_map_->getInflateOccupancy(pos_t[j]);
            if(j > 0 && occupied){
                Eigen::Vector3d sub = pos_t[j] - pos_t[j-1];
                Eigen::Vector2d dir(sub(0),sub(1));
                Eigen::Vector3d tlpos(pos_t[j]), trpos(pos_t[j]);
                int loccupied, roccupied;
                do{
                    if(abs(dir(0)) > 0.05 && abs(dir(1)) > 0.05){
                        tlpos(0) = tlpos(0) + 0.1;
                        tlpos(1) = dir(1)/dir(0) * 0.1 + tlpos(1);
                        trpos(0) = trpos(0) - 0.1;
                        trpos(1) = dir(1)/dir(0) * (-0.1) + tlpos(1);
                    }else if(abs(dir(1)) > 0.05){
                        tlpos(1) = tlpos(1) + 0.1;
                        trpos(1) = trpos(1) - 0.1;
                    }else if(abs(dir(0) > 0.05)){
                        tlpos(0) = tlpos(0) + 0.1;
                        trpos(0) = trpos(0) - 0.1;
                    }
                    loccupied = planner_manager_->sdf_map_->getInflateOccupancy(tlpos);
                    roccupied = planner_manager_->sdf_map_->getInflateOccupancy(trpos);
                }while(loccupied && roccupied);
                if(!loccupied){
                    use_temp = 0.0;
                    use_temp_arr_[j] = use_temp;
                    break;
                }
                else if(!roccupied){
                    use_temp = 1.0;
        ...
        for(int i = 0;i < 5; i++){
            cout << " use_temp_arr_[ " << i << "]  "  <<use_temp_arr_[i] << endl;
    ...
    if(have_target_ ) {
        switch (hh) {
            case 0: {
                if(odom_pos_(0) > -0.5 && odom_pos_(0) < 1.0){
                    cout << "odom_pos \n" << odom_pos_ << endl;
                    cout << "external force \n" << external_force_ << endl;
                    cout << "first force \n" << external_force_remain_ << endl;
                    cout << "wight_use_extforce_   " << wight_use_extforce_ << endl;
                    hh++;
                }
                break;
            }
            case 1 : {
                if(odom_pos_(0) > 2.0 && odom_pos_(0) < 2.5){
                    cout << "odom_pos \n" << odom_pos_ << endl;
                    cout << "external force \n" << external_force_ << endl;
                    cout << "second force \n" << external_force_remain_ << endl;
                    cout << "wight_use_extforce_   " << wight_use_extforce_ << endl;
...
void IntelligentReplanFsm::externalforceGenCallback(const ros::TimerEvent& e){
    ...
    if(have_odom_ && odom_pos_(0) > -1.5 && odom_pos_(0) < 3.5){
        ...
        if(times > 0){
            ...
            external_force_(1) = -2.0 + y_dist * 0.3;

            if(external_force_(0) < 0 )
                external_force_(0) = 0.0;
            if(external_force_(1) > 0)
                external_force_(1) = 0.0;
        }
    }else {
        external_force_ << 0, 0, 0;
    }
    ...
    extforce_comp.x = (1 - wight_use_extforce_) * external_force_(0) * 0.98;
    extforce_comp.y = (1 - wight_use_extforce_) * external_force_(1) * 0.98;
    extforce_comp.z = (1 - wight_use_extforce_) * external_force_(2) * 0.98;
...
void IntelligentReplanFsm::cmdCallback(const ros::TimerEvent& e){
    ...
    /*
    static int count = 0;
    if(count < ego_pos_.size()){
        ego_history_pos_.push_back(ego_pos_[count]);
        ego_history_vel_.push_back(ego_vel_[count]);
        draw_history_traj_ego();
        count++;
    }
  */
    ...
    if (have_traj_)
    {
        ...
        if (mpc_traj_pos_.size() == 0){
            ...
            acc_L = mpc_traj_acc_[k];
            acc_R = mpc_traj_acc_[k+1];
            vel_L = mpc_traj_vel_[k];
            vel_R = mpc_traj_vel_[k+1];
            pos_L = mpc_traj_pos_[k];
            pos_R = mpc_traj_pos_[k+1];
        }else {
            t = (ros::Time::now() - tMpc1_).toSec();
            k = floor(t/mpc_delta_T_);
            // cout <<"[2]  k: " << k << "  t:  " << t << endl;
...
void IntelligentReplanFsm::draw_traj_ego(){
        static bool end = true;
        ifstream file("/home/lsl/egopoint.txt", ios::in);
        if(end) {
            ...
            while (getline(file, temp)) {
                ...
                if (j % 2 == 0) {
                    ...
                    pos << pv[0], pv[1], pv[2];
                    //cout << "pos \n" << pos << endl;
                    ego_pos_.push_back(pos);
                    if ((pos - end_pt_).norm() < 0.2) {
                        end = false;
                        break;
                    }
                ...
                if (j % 2 == 1) {
                    ...
                    vel << pv[0], pv[1], pv[2];
                    ego_vel_.push_back(vel);
                    // cout << "vel \n" << vel << endl;
                }
                j++;
            }
        }
        //cout << "our_pos_[our_pos_.size() -1 ]"   << our_pos_[our_pos_.size() -1 ]  << endl;
        //cout << "j   "  << j << endl;
        // cout << pos << endl;
        file.close();
...
void IntelligentReplanFsm::draw_history_traj_ego(){
        unsigned int sp = ego_history_pos_.size(), sv = ego_history_vel_.size();
        //cout << "sp "<<sp << endl;
        int size = sp<=sv ? sp : sv;
        visualization_msgs::Marker mk;
        mk.header.frame_id = "world";
        ...
        mk.scale.x = 0.15;
        for (int i = 0; i < size; i++){
            Eigen::Vector3d v = ego_history_vel_[i];
            double v_norm = v.norm();
            // cout << "v  :  " << v_norm << endl;
            double color_num;
            if(v_norm < v_min_)
            ...
            geometry_msgs::Point pt;
            pt.x = ego_history_pos_[i](0);
            pt.y = ego_history_pos_[i](1);
            pt.z = ego_history_pos_[i](2);
            mk.points.push_back(pt);

        }
        ego_traj_pub_.publish(mk);
        // ros::Duration(0.001).sleep();
    }

void IntelligentReplanFsm::draw_opt_ego(){
    static bool end = true;
    ifstream file("/home/lsl/ego_opt_point.txt", ios::in);
    if(end) {
        Eigen::Vector3d pos, vel;
        std::string temp;
        int j = 0;
...
void IntelligentReplanFsm::displayOptimalList(vector<Eigen::Vector3d> list, int id)
{
    Eigen::Vector4d color(1, 0, 0, 1);
    displayMarkerList(ego_opt_pub_, list, 0.15, color, id);
}

void IntelligentReplanFsm::displayMarkerList(ros::Publisher &pub, const vector<Eigen::Vector3d> &list, double scale,
