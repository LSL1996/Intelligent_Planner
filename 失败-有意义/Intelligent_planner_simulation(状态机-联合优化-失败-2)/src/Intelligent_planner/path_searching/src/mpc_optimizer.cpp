...
std::vector <std::vector<Eigen::Vector3d>> mpc_optimizer::mpcOptimizeTraj(Eigen::MatrixXd start_state,
    ...
    if (if_external_force_adaptive_) {
        A_external_force_ = K_ * external_force;
        alpha6_ = alpha6_;
    } else {
...
void mpc_optimizer::calCostFunctionandGradient() {
    ...
    for (int i = 0; i < N_; i++) {
        if (i > 0) {
            ...
            if (abs(dist) < dist_0_) {
                ...
                if(if_external_force_adaptive_) { // if_external_force_adaptive_
                    VectorXd temp = VectorXd::Zero(3);
                    temp << 3 * pow(-dist + dist_0_, 2) * (-1) * dist_grad(0) * C_.row(3 * i) ,
                            3 * pow(-dist + dist_0_, 2) * (-1) * dist_grad(1) * C_.row(3 * i) ,
                            3 * pow(-dist + dist_0_, 2) * (-1) * dist_grad(2) * C_.row(3 * i) ;
                    Gradient_cf += temp;
            ...
            if(if_external_force_adaptive_){ //if_external_force_adaptive_
                VectorXd temp = VectorXd::Zero(3);
                //cout << "if_external_force_adaptive_     " << if_external_force_adaptive_ << endl;
                temp << (calRangeGrad(vel(0), vel_lower_, vel_upper_) * C_.row(3 * i + 1) + calRangeGrad(acc(0), acc_lower_, acc_upper_) * C_.row(3 * i + 2)),
                        (calRangeGrad(vel(1), vel_lower_, vel_upper_) * C_.row(3 * i + 1) + calRangeGrad(acc(1), acc_lower_, acc_upper_) * C_.row(3 * i + 2)),
                        (calRangeGrad(vel(2), vel_lower_, vel_upper_) * C_.row(3 * i + 1) + calRangeGrad(acc(2), acc_lower_, acc_upper_) * C_.row(3 * i + 2));
        ...
        if(false) {
            ...
            if (i == N_ - 1) {
                for (int j = 0; j < N_ ; j++) {
                    ...
                    VectorXd temp = VectorXd::Zero(3);
                    temp << Gradient_fdirx_t(i) * C_.row(3 * j),
                            Gradient_fdiry_t(i) * C_.row(3 * j),
                            0.0;
                    Gradient_fdirf += temp;
    ...
    if(if_external_force_adaptive_){
        for(int i = 0; i < 3 ; i++) {
            ...
            fr += amiga1_ * pow((K_ * input_f_(i) - A_external_force_(i)), 2) + amiga2_ * cost;
            Gradient_fr(i) = amiga1_ * 2 * (K_ * input_f_(i)  - A_external_force_(i)) * K_ + amiga2_ * grad;
        }
        //cout << " fr            " << fr << endl;
        //cout << " Gradient_fr \n" << Gradient_fr << endl;
    }
