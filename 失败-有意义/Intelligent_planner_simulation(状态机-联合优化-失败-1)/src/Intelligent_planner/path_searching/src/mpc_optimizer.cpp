...
void mpc_optimizer::setParam(ros::NodeHandle &nh) {
    ...
    nh.param("optimization/amiga1", amiga1_, -1.0);
    nh.param("optimization/amiga2", amiga2_, -1.0);
...
std::vector <std::vector<Eigen::Vector3d>> mpc_optimizer::mpcOptimizeTraj(Eigen::MatrixXd start_state,
    ...
    external_force_remain_ =  external_force;
...
void mpc_optimizer::optimize() {
    ...
    for(int i = 3 * (N_ - 1); i < 3 * (N_ - 1) + 3; i++){
        q[i] = input_f_(i - 3 * (N_ - 1));
    }
...
void mpc_optimizer::stateEquations() {
    if( if_external_force_adaptive_) {
        state_x_ = A_ * input_x_ + B_ * start_state_.col(0) + C_ * input_f_(0) / input_f_.norm();
        state_y_ = A_ * input_y_ + B_ * start_state_.col(1) + C_ * input_f_(1) / input_f_.norm();
        state_z_ = A_ * input_z_ + B_ * start_state_.col(2) + C_ * input_f_(2) / input_f_.norm();
    }else{
...
void mpc_optimizer::resetInputInital() {
    ...
    if (local_result_path_.size() > 1) {
        ...
        if(external_force_remain_.norm() > 0.5)
...
void mpc_optimizer::calCostFunctionandGradient() {
    ...
    double ft, fu, fc, fvelacc, fjerk, fef, fdir, fr;
    ft = fu = fc = fvelacc = fjerk = fef = fdir = fr = 0.0;
    ...
    VectorXd Gradient_tx, Gradient_ty, Gradient_tz, Gradient_tf;
    Gradient_tx = Gradient_ty = Gradient_tz = VectorXd::Zero(N_ - 1); Gradient_tf = VectorXd::Zero(3);

    VectorXd Gradient_cx, Gradient_cy, Gradient_cz, Gradient_cf;
    Gradient_cx = Gradient_cy = Gradient_cz = VectorXd::Zero(N_ - 1); Gradient_cf = VectorXd::Zero(3);

    VectorXd Gradient_velx, Gradient_vely, Gradient_velz, Gradient_accx, Gradient_accy, Gradient_accz, Gradient_fvelacc;
    Gradient_velx = Gradient_vely = Gradient_velz = VectorXd::Zero(N_ - 1);
    Gradient_accx = Gradient_accy = Gradient_accz = VectorXd::Zero(N_ - 1); Gradient_fvelacc = VectorXd::Zero(3);

    VectorXd Gradient_jerkx, Gradient_jerky, Gradient_jerkz;
    Gradient_jerkx = Gradient_jerky = Gradient_jerkz = VectorXd::Zero(N_ - 1);
    //---------------------------------------------------------//
    VectorXd d_f_beta1_2_x,d_f_beta1_2_y,d_f_beta1_2_z, d_f_beta1_2_f;
    d_f_beta1_2_x = d_f_beta1_2_y = d_f_beta1_2_z = VectorXd::Zero(N_ - 1); d_f_beta1_2_f = VectorXd::Zero(3);
    VectorXd Gradient_fvx, Gradient_fvy, Gradient_fvz, Gradient_fax, Gradient_fay, Gradient_faz, Gradient_fvf, Gradient_faf;
    Gradient_fvx = Gradient_fvy = Gradient_fvz = Gradient_fax = Gradient_fay = Gradient_faz = VectorXd::Zero(N_ - 1);
    Gradient_fvf = Gradient_faf = VectorXd::Zero(3);

    VectorXd Gradient_fdirx,Gradient_fdiry, Gradient_fdirf;
    Gradient_fdirx = Gradient_fdiry = VectorXd::Zero(N_-1); Gradient_fdirf = VectorXd::Zero(3);
    ...
    //-------------------------------------------------------//
    VectorXd Gradient_fr;
    Gradient_fr = VectorXd::Zero(3);
    //-------------------------------------------------------//
    ...
    for (int i = 0; i < N_; i++) {
        if (i > 0) {
            ...
            if(if_external_force_adaptive_){ //if_external_force_adaptive_
                //cout << "if_external_force_adaptive_       "<< if_external_force_adaptive_ << endl;
                VectorXd temp = VectorXd::Zero(3);
                temp <<  2 * (pos(0) - path_pos(0)) * C_.row(3 * i) * (pow(input_f_.norm(),2) - pow(input_f_(0),2))/pow(input_f_.norm(), 3),
                         2 * (pos(1) - path_pos(1)) * C_.row(3 * i) * (pow(input_f_.norm(),2) - pow(input_f_(1),2))/pow(input_f_.norm(), 3),
                         2 * (pos(2) - path_pos(2)) * C_.row(3 * i) * (pow(input_f_.norm(),2) - pow(input_f_(2),2))/pow(input_f_.norm(), 3);
                Gradient_tf += temp;
               // cout << " Gradient_tf   \n "<< Gradient_tf  << endl;
            }
            ...
            if (abs(dist) < dist_0_) {
                ...
                if(if_external_force_adaptive_) { // if_external_force_adaptive_
                    VectorXd temp = VectorXd::Zero(3);
                    temp << 3 * pow(-dist + dist_0_, 2) * (-1) * dist_grad(0) * C_.row(3 * i) * (pow(input_f_.norm(),2) - pow(input_f_(0),2))/pow(input_f_.norm(), 3),
                            3 * pow(-dist + dist_0_, 2) * (-1) * dist_grad(1) * C_.row(3 * i) * (pow(input_f_.norm(),2) - pow(input_f_(1),2))/pow(input_f_.norm(), 3),
                            3 * pow(-dist + dist_0_, 2) * (-1) * dist_grad(2) * C_.row(3 * i) * (pow(input_f_.norm(),2) - pow(input_f_(2),2))/pow(input_f_.norm(), 3);
                    Gradient_cf += temp;
            ...
            if(if_external_force_adaptive_){ //if_external_force_adaptive_
                VectorXd temp = VectorXd::Zero(3);
                //cout << "if_external_force_adaptive_     " << if_external_force_adaptive_ << endl;
                temp << (calRangeGrad(vel(0), vel_lower_, vel_upper_) * C_.row(3 * i + 1) + calRangeGrad(acc(0), acc_lower_, acc_upper_) * C_.row(3 * i + 2)) * (pow(input_f_.norm(),2) - pow(input_f_(0),2))/pow(input_f_.norm(), 3),
                        (calRangeGrad(vel(1), vel_lower_, vel_upper_) * C_.row(3 * i + 1) + calRangeGrad(acc(1), acc_lower_, acc_upper_) * C_.row(3 * i + 2)) * (pow(input_f_.norm(),2) - pow(input_f_(1),2))/pow(input_f_.norm(), 3),
                        (calRangeGrad(vel(2), vel_lower_, vel_upper_) * C_.row(3 * i + 1) + calRangeGrad(acc(2), acc_lower_, acc_upper_) * C_.row(3 * i + 2)) * (pow(input_f_.norm(),2) - pow(input_f_(2),2))/pow(input_f_.norm(), 3);
                Gradient_fvelacc +=  temp;

                //cout << " Gradient_fvelacc    "<< Gradient_fvelacc  << endl;
            }
            /* minimum input jerk */
            // fjerk
            if (i < N_ - 1) {
            ...
            if (false) {  //if_external_force_adaptive_
                if (abs(dist) < dist_1_) {
                    ...
                    if (abs(acc(0)) > acc_upper_) {
                        ...
                        //VectorXd d_f_beta1_2_fx;
                        //d_f_beta1_2_fx = 1.0 * C_.row(3 * i + 2) * pow(dist - dist_1_, 2) * fef_vel * fef_acc;
                        //d_f_beta1_2_f(0) = acc(0) > 0 ? d_f_beta1_2_fx(0) : -d_f_beta1_2_fx(0);
                        ...
                        //VectorXd d_f_beta1_2_fx;
                        //d_f_beta1_2_fx = 1.0 * exp(abs(acc(0)) - acc_upper_) * C_.row(3 * i + 2) * pow(dist - dist_1_, 2) * fef_vel * fef_acc;
                        //d_f_beta1_2_f(0) = acc(0) > 0 ? d_f_beta1_2_fx(0) : -d_f_beta1_2_fx(0);
                    }
                    ...
                    if (abs(acc(1)) > acc_upper_) {
                        ...
                        //VectorXd d_f_beta1_2_fy;
                        //d_f_beta1_2_fy = 1.0 * C_.row(3 * i + 2) * pow(dist - dist_1_, 2) * fef_vel * fef_acc;
                        //d_f_beta1_2_f(1) = acc(1) > 0 ? d_f_beta1_2_fy(0) : -d_f_beta1_2_fy(0);
                    } else if (abs(acc(1)) > 3.0) {
                        d_f_beta1_2_y = 1.0 * exp(abs(acc(1)) - acc_upper_) * A_.row(3 * i + 2) * pow(dist - dist_1_, 2) * fef_vel * fef_acc;
                        d_f_beta1_2_y = acc(1) > 0 ? d_f_beta1_2_y : -d_f_beta1_2_y;

                        //VectorXd d_f_beta1_2_fy;
                        //d_f_beta1_2_fy = 1.0 * exp(abs(acc(1)) - acc_upper_) * C_.row(3 * i + 2) * pow(dist - dist_1_, 2) * fef_vel * fef_acc;
                        //d_f_beta1_2_f(1) = acc(1) > 0 ? d_f_beta1_2_fy(0) : -d_f_beta1_2_fy(0);
                    }
                    ...
                    if (abs(acc(2)) > 0.8) {
                        ...
                        //VectorXd d_f_beta1_2_fz;
                        //d_f_beta1_2_fz = 0.5 * exp(abs(acc(2)) - 0.8) * C_.row(3 * i + 2) * pow(dist - dist_1_, 2) * fef_vel * fef_acc;
                        //d_f_beta1_2_f(2) = acc(2) > 0 ? d_f_beta1_2_fz(0) : -d_f_beta1_2_fz(0);
                    }
                    ...
                    /*
                    VectorXd temp1;
                    temp1 << f_beta1 * fef_acc * 2 * (dist - dist_1_) * dist_grad(0) * C_.row(3 * i) * fef_vel +
                            f_beta1 * fef_acc * pow(dist - dist_1_, 2) * vel(0) * (vel.norm() - vel_limit_) /
                            vel.norm() * C_.row(3 * i + 1) / (1 + fef_vel_temp) / log(M_),
                            f_beta1 * fef_acc * 2 * (dist - dist_1_) * dist_grad(1) * C_.row(3 * i) * fef_vel +
                            f_beta1 * fef_acc * pow(dist - dist_1_, 2) * vel(1) * (vel.norm() - vel_limit_) /
                            vel.norm() * C_.row(3 * i + 1) / (1 + fef_vel_temp) / log(M_),
                            f_beta1 * fef_acc * 2 * (dist - dist_1_) * dist_grad(2) * C_.row(3 * i) * fef_vel +
                            f_beta1 * fef_acc * pow(dist - dist_1_, 2) * vel(2) * (vel.norm() - vel_limit_) /
                            vel.norm() * C_.row(3 * i + 1) / (1 + fef_vel_temp) / log(M_);
                    Gradient_fvf += temp1;
                    */
                    ...
                    /*
                    VectorXd temp2;
                    temp2 << f_beta1 * pow(dist - dist_1_, 2) * fef_vel * acc(0) *
                            (acc.norm() - sqrt(acc_upper_ * acc_upper_ * 3)) / acc.norm() * C_.row(3 * i + 2) /
                            (1 + fef_acc_tmp) / log(Ma_),
                            f_beta1 * pow(dist - dist_1_, 2) * fef_vel * acc(1) *
                            (acc.norm() - sqrt(acc_upper_ * acc_upper_ * 3)) / acc.norm() * C_.row(3 * i + 2) /
                            (1 + fef_acc_tmp) / log(Ma_),
                            f_beta1 * pow(dist - dist_1_, 2) * fef_vel * acc(2) *
                            (acc.norm() - sqrt(acc_upper_ * acc_upper_ * 3)) / acc.norm() * C_.row(3 * i + 2) /
                            (1 + fef_acc_tmp) / log(Ma_);
                    Gradient_faf += temp2;
                    Gradient_faf += d_f_beta1_2_f;
                    */
               }
            }
        }
        ...
        if(if_external_force_adaptive_) {
            ...
            if (i == N_ - 1) {
                for (int j = 0; j < N_ ; j++) {
                    ...
                    VectorXd temp = VectorXd::Zero(3);
                    temp << Gradient_fdirx_t(i) * C_.row(3 * j) * (pow(input_f_.norm(),2) - pow(input_f_(0),2))/pow(input_f_.norm(), 3),
                            Gradient_fdiry_t(i) * C_.row(3 * j) * (pow(input_f_.norm(),2) - pow(input_f_(1),2))/pow(input_f_.norm(), 3),
                            0.0;
                    Gradient_fdirf += temp;
    ...
    if(if_external_force_adaptive_){
        for(int i = 0; i < 3 ; i++) {
            ...
            //cout << "grad  " << grad  << endl;
            //cout << "cost  " << cost  << endl;
            //cout << "input_f_.norm()    " << input_f_.norm()  <<endl;
            if (input_f_.norm() > 0.01) {
                fr += amiga1_ * pow((K_ * input_f_(i) / input_f_.norm() - A_external_force_(i)), 2) + amiga2_ * cost;
                Gradient_fr(i) = amiga1_ * 2 * (K_ * input_f_(i) / input_f_.norm() - A_external_force_(i)) * K_ *
                                 (pow(input_f_.norm(), 2) - pow(input_f_(i), 2)) / pow(input_f_.norm(), 3) +
                                 amiga2_ * grad * (pow(input_f_.norm(),2) - pow(input_f_(i),2))/pow(input_f_.norm(), 3);
            }
        }
        //cout << " fr            " << fr << endl;
        //cout << " Gradient_fr \n" << Gradient_fr << endl;
    ...
    f_ = alpha1_ * ft + alpha2_ * fu + alpha3_ * fc + alpha4_ * fvelacc + alpha5_ * fjerk  + alpha6_ * fef + alpha7_ * fdir + alpha8_ * fr;
    ...
    Gradient_f_ = alpha1_ * Gradient_tf +
                  alpha3_ * Gradient_cf +
                  alpha4_ * Gradient_fvelacc +
                  alpha6_ * Gradient_faf +
                  alpha7_ * Gradient_fdirf +
                  alpha8_ * Gradient_fr;
}
...
void mpc_optimizer::getExtforceGardCost(double x, double u, double grad, double cost){
    double lx,hx;
    if(x > 0){
        lx = 0.0;
        hx = x;
