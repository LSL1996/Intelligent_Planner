...
void IntelligentPlannerManager::initPlanModules(ros::NodeHandle& nh){
    ...
    high_mpc_optimizer_.reset(new high_mpc_optimizer);
    high_mpc_optimizer_->setParam(nh);
    high_mpc_optimizer_->setEnvironment(edt_environment_);
}
...
bool IntelligentPlannerManager::highMpc(Eigen::MatrixXd start_state, bool if_external_force_adaptive, Eigen::Vector3d external_force){
    ...
    mpc_best_variable_ = mpc_optimizer_->best_variable_;
    return true;
}

bool IntelligentPlannerManager::highhighMpc(Eigen::MatrixXd start_state,  Eigen::Vector3d external_force_remain){
    high_mpc_optimizer_-> external_acc_ = external_acc_;
    high_mpc_traj_ = high_mpc_optimizer_->highmpcOptimizeTraj(start_state, mpc_traj_pos_, mpc_traj_vel_, mpc_traj_acc_, mpc_best_variable_, external_force_remain);

    high_mpc_traj_pos_     = high_mpc_traj_[0];
    high_mpc_traj_vel_     = high_mpc_traj_[1];
    high_mpc_traj_acc_     = high_mpc_traj_[2];
    return true;
}
...
void IntelligentPlannerManager::resetMPCinitial(){
    ...
    high_mpc_optimizer_->resetInputInital();
}

void IntelligentPlannerManager::getOptimizedExtforce(Eigen::Vector3d& optimizedforce){
    for(int i = 0; i < 3; i++) {
        optimizedforce(i) = high_mpc_optimizer_->best_variable_[i];
    }
}
